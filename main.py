...
def load_model():
    ...
    # Use your fine-tuned weights if present, else fall back to pretrained
    model_path = "best.pt" if os.path.exists("best.pt") else "yolov8s-cls.pt"
    model = YOLO(model_path)  # ultralytics auto-downloads yolov8s-cls.pt if not found
    return model
...
def run_predict(model, img_bgr):
    """Run YOLOv8 prediction and return (probs array, class names list)."""
...
if mode == "📷 Image":
    ...
    if uploaded:
        # Decode image from bytes — works reliably on all platforms
        ...
        # Load model AFTER image is ready (avoids blank spinner on first load)
        model = load_model()
...
# ══════════════════════ VIDEO MODE ════════════════════════════════════════════
    ...
    if uploaded_video:
        ...
        output_path = tmp_in_path.replace(".mp4", "_out.mp4")
        ...
        cap.release(); out.release(); os.unlink(tmp_in_path)
        progress.progress(1.0)
        status.text(f"✅ Done — {count} frames processed.")
...
# ══════════════════════ ABOUT MODE ════════════════════════════════════════════
    ...
    with col1:
        ...
        st.markdown("""
        Uses **YOLOv8s-cls** to classify vehicle images and video frames into
        four categories in real time.
